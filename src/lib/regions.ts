...
//
// `active: true` means the region is publicly accessible. Inactive regions
// are gated in middleware — only admin users (super/editor) can visit the
// subdomain; everyone else is redirected to www. Flip a region to `true`
// here when you're ready to launch it to the public.
...
/**
 * Whether a region is publicly accessible. Inactive regions are visible to
 * admin users (super/editor) only — enforced in middleware.
 */
export function isRegionActive(code: string): boolean {
  return REGIONS.some(r => r.code === code && r.active)
}
