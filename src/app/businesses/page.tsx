...
export default function BusinessesPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <p className="text-gray-500 mb-6">NY/NJ 한인 비즈니스를 찾아보세요</p>
          ...
          {loading ? (
            ...
            <>
                ...
                {paginated.map((b: any) => {
                  ...
                  return (
                    <Link key={b.id} href={`/business/${b.id}`}>
                      <div className={`rounded-lg p-4 hover:shadow-lg transition-all ${style.border} ${style.bg} ${style.glow}`}>
                        {/* 플랜 뱃지 */}
              ...
              <p className="text-center text-xs text-gray-400 mt-3">
                총 {filtered.length}개 업체 중 {(currentPage - 1) * ITEMS_PER_PAGE + 1}-{Math.min(currentPage * ITEMS_PER_PAGE, filtered.length)}
              </p>
            </>
          )}
        </div>
        ...
        <aside className="hidden md:block">
          <div className="sticky top-20 space-y-4">
            ...
            <div className="border rounded-lg p-4 bg-gray-50">
              <h3 className="font-bold text-sm mb-2">내 비즈니스 등록</h3>
              <p className="text-xs text-gray-500 mb-3">이동네에 비즈니스를 등록하고 고객을 만나보세요.</p>
              <Link href="/business-register" className="block text-center bg-black text-white text-sm py-2 rounded-lg hover:bg-gray-800">
                무료 등록하기
              </Link>
            </div>
          </div>
